...
int cortex_m_add_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	struct cortex_m_common *cortex_m = target_to_cm(target);
	LOG_DEBUG("cortex_m->auto_bp_type: %d, breakpoint->address: 0x%x, breakpoint->type: %d, breakpoint->length: %d, cortex_m->fp_code_available: %d", cortex_m->auto_bp_type, breakpoint->address, breakpoint->type, breakpoint->length, cortex_m->fp_code_available);
	if (cortex_m->auto_bp_type)
		breakpoint->type = BKPT_TYPE_BY_ADDR(breakpoint->address);
